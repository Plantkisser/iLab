...
namespace geom
{
	...
	template <typename T> std:: vector<Point<T>> GetParalIntersection(Point <T> p1, Point <T> p2, Point <T> p3, Point <T> p4, bool isFirst = false);

	template <typename T> bool CheckIntersection(Point <T> p1, Point <T> p2, Point <T> p3, Point <T> p4, bool isFirst = false, bool isPrevZero = false);
